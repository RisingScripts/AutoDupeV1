...
if PlayerGui:FindFirstChild("DeepWipe_Bypass") then PlayerGui.DeepWipe_Bypass:Destroy() end

local screenGui = Instance.new("ScreenGui", PlayerGui)
screenGui.Name = "DeepWipe_Bypass"
screenGui.ResetOnSpawn = false
local MainColor = Color3.fromRGB(255, 255, 0)
local AltColor = Color3.fromRGB(0, 0, 255) 

local main = Instance.new("Frame", screenGui); main.Size = UDim2.new(0, 240, 0, 240); main.Position = UDim2.new(0.5, -120, 0.5, -120); main.BackgroundColor3 = Color3.fromRGB(15, 15, 20); Instance.new("UICorner", main).CornerRadius = UDim.new(0, 10); local stroke = Instance.new("UIStroke", main); stroke.Color = MainColor; stroke.Thickness = 2
local altMain = Instance.new("Frame", screenGui); altMain.Size = UDim2.new(0, 240, 0, 240); altMain.Position = main.Position; altMain.BackgroundColor3 = Color3.fromRGB(15, 15, 20); altMain.Visible = false; Instance.new("UICorner", altMain).CornerRadius = UDim.new(0, 10); local altStroke = Instance.new("UIStroke", altMain); altStroke.Color = AltColor; altStroke.Thickness = 2

-- [[ CONSISTENCY LABEL (PLACED OVER STATUS) ]] --
local consistencyLabel = Instance.new("TextLabel", main)
consistencyLabel.Size = UDim2.new(1, 0, 0, 20)
consistencyLabel.Position = UDim2.new(0, 0, 0, 35)
consistencyLabel.BackgroundTransparency = 1
consistencyLabel.Text = "EQUIP ITEM FOR BETTER CONSISTENCY"
consistencyLabel.TextColor3 = Color3.fromRGB(255, 0, 0)
consistencyLabel.Font = "GothamBold"
consistencyLabel.TextSize = 11
consistencyLabel.Visible = false

local helpBox = Instance.new("Frame", altMain); helpBox.Size = UDim2.new(1, 0, 1, 0); helpBox.BackgroundColor3 = Color3.fromRGB(10, 10, 15); helpBox.BorderSizePixel = 0; helpBox.Visible = false; helpBox.ZIndex = 10; Instance.new("UICorner", helpBox)
local helpLabel = Instance.new("TextLabel", helpBox); helpLabel.Size = UDim2.new(1, -20, 1, -40); helpLabel.Position = UDim2.new(0, 10, 0, 10); helpLabel.BackgroundTransparency = 1; helpLabel.TextColor3 = Color3.new(1, 1, 1); helpLabel.Font = "GothamBold"; helpLabel.TextSize = 14; helpLabel.TextWrapped = true; helpLabel.Text = "You can have up to 3 guns/chains in your inventory when pressing the safe Dupe button it will dupe them all."; helpLabel.ZIndex = 11
local helpBack = Instance.new("TextButton", helpBox); helpBack.Size = UDim2.new(0, 80, 0, 25); helpBack.Position = UDim2.new(0.5, -40, 1, -30); helpBack.BackgroundColor3 = AltColor; helpBack.Text = "OK"; helpBack.TextColor3 = Color3.new(1, 1, 1); helpBack.Font = "GothamBold"; helpBack.ZIndex = 11; Instance.new("UICorner", helpBack)
local helpCircle = Instance.new("TextButton", altMain); helpCircle.Size = UDim2.new(0, 25, 0, 25); helpCircle.Position = UDim2.new(0, 5, 0, 5); helpCircle.BackgroundColor3 = Color3.fromRGB(30, 30, 35); helpCircle.Text = "?"; helpCircle.TextColor3 = Color3.new(1, 1, 1); helpCircle.Font = "GothamBold"; helpCircle.TextSize = 14; Instance.new("UICorner", helpCircle).CornerRadius = UDim.new(1, 0); local hStroke = Instance.new("UIStroke", helpCircle); hStroke.Color = AltColor; hStroke.Thickness = 1.5
...
local function createHeader(parent, color)
    local title = Instance.new("TextLabel", parent); title.Size = UDim2.new(1, 0, 0, 30); title.BackgroundTransparency = 1; title.Text = "RisingScripts"; title.TextColor3 = color; title.Font = "GothamBold"; title.TextSize = 18
    local st = Instance.new("TextLabel", parent); st.Name = "StatusLabel"; st.Size = UDim2.new(1, 0, 0, 20); st.Position = UDim2.new(0,0,0,35); st.BackgroundTransparency = 1; st.Text = "STATUS: READY"; st.TextColor3 = Color3.new(1,1,1); st.Font = "GothamBold"; st.TextSize = 11
    return st
end
local status = createHeader(main, MainColor); local altStatus = createHeader(altMain, AltColor)
...
RunService.RenderStepped:Connect(function()
    ...
    local isEquipped = LocalPlayer.Character and LocalPlayer.Character:FindFirstChildOfClass("Tool")
    
    if successCounter > 0 then
        -- We have successes, show the counter
        status.Text = "Duped Items: " .. successCounter
        status.TextColor3 = Color3.fromRGB(0, 255, 127)

        if successCounter >= 2 and not isEquipped then
            status.Visible = false
            consistencyLabel.Visible = true
        else
            status.Visible = true
            consistencyLabel.Visible = false
        end
    else
        -- Active but waiting for first success (Initialization is hidden)
        status.Visible = false
        consistencyLabel.Visible = false
    end
end)
...
local function createBtn(parent, text, pos, color)
    local b = Instance.new("TextButton", parent); b.Size = UDim2.new(0, 210, 0, 38); b.Position = pos; b.BackgroundColor3 = color; b.Text = text; b.Font = "GothamBold"; b.TextSize = 14; b.TextColor3 = (color == MainColor and Color3.new(0,0,0) or Color3.new(1,1,1)); Instance.new("UICorner", b).CornerRadius = UDim.new(0, 8)
    return b
end

local dupeBtn = createBtn(main, "START DUPE", UDim2.new(0.5, -105, 0, 60), MainColor)
local killBtn = createBtn(main, "KILL ALL SERVER", UDim2.new(0.5, -105, 0, 110), AltColor)
local toggBtn = createBtn(main, "Toggles", UDim2.new(0.5, -105, 0, 160), MainColor)

local safeDupe = createBtn(altMain, "Safe Dupe", UDim2.new(0.5, -105, 0, 60), AltColor)
local safeTp = createBtn(altMain, "Safe Tp", UDim2.new(0.5, -105, 0, 110), AltColor)
local autoDrop = createBtn(altMain, "Auto Drop", UDim2.new(0.5, -105, 0, 160), AltColor)

-- [[ INTEGRATED FULL HYBRID DUPE LOGIC ]] --

dupeBtn.MouseButton1Click:Connect(function()
    _G.DupeActive = not _G.DupeActive
    currentSession = currentSession + 1
    local mySession = currentSession
    
    dupeBtn.Text = _G.DupeActive and "STOP DUPE" or "START DUPE"
    dupeBtn.BackgroundColor3 = _G.DupeActive and Color3.new(0.7,0,0) or MainColor
            ...
            while _G.DupeActive and mySession == currentSession do
                ...
                if tool and not Blacklist[tool.Name] then
                    ...
                    if currentCount > startCount then
                        ...
                        if successCounter >= 2 then
                           if not hasDoneForceEquip then
                               if masterObject and masterObject.Parent == bp then
                                   hum:EquipTool(masterObject)
                               end
...
local tFrame = Instance.new("Frame", screenGui); tFrame.Size = UDim2.new(0, 280, 0, 320); tFrame.Position = main.Position; tFrame.BackgroundColor3 = Color3.fromRGB(15,15,20); tFrame.Visible = false; Instance.new("UICorner", tFrame); local tStr = Instance.new("UIStroke", tFrame); tStr.Color = MainColor; tStr.Thickness = 2
...
makeDraggable(main); makeDraggable(altMain); makeDraggable(tFrame)
